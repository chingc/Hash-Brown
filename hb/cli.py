...
@click.option("-a", "--algorithm", required=True, type=click.Choice(Checksum.supported()))
...
def compute(algorithm: str, paths: List[str], progress: bool) -> None:
    ...
    for path in paths:
        for filename in iglob(path, recursive=True):
            if isfile(filename):
                checksum = Checksum(filename, show_progress=progress)
                if algorithm == "blake2b":
                    result = checksum.blake2b()
                elif algorithm == "blake2s":
                    result = checksum.blake2s()
                elif algorithm == "md5":
                    result = checksum.md5()
                elif algorithm == "sha1":
                    result = checksum.sha1()
                elif algorithm == "sha224":
                    result = checksum.sha224()
                elif algorithm == "sha256":
                    result = checksum.sha256()
                elif algorithm == "sha384":
                    result = checksum.sha384()
                elif algorithm == "sha512":
                    result = checksum.sha512()
                elif algorithm == "adler32":
                    result = checksum.adler32()
                else:
                    result = checksum.crc32()
                click.echo(f"{algorithm} ({filename}) = {result}")
                count += 1
        if not count:
